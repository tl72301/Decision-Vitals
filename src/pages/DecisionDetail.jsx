    ...
    <div>
      ...
      <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            {decision.title && decision.statement && (
              <p className="mt-1 text-sm text-slate-600">{decision.statement}</p>
            )}
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4">
            ...
            {reports.map((r) => (
              <Link
                ...
                className="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium text-slate-700 ring-1 ring-inset ring-slate-200 hover:bg-slate-200"
              >
                Review #{r.runNumber}
              </Link>
            ))}
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-2">
        <section>
          ...
          {locked && (
            <p className="mb-3 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-500 ring-1 ring-inset ring-slate-200">
        ...
        <section>
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <span className="text-xs text-slate-400">{evidence.length} total</span>
          </div>
      ...
      {/* Run review */}
      <div className="mt-8 flex flex-col items-start gap-2 border-t border-slate-200 pt-6">
        {canReview ? (
          <Link
            to={`/decision/${id}/run`}
          ...
          <>
            <button
              type="button"
              disabled
              className="inline-flex cursor-not-allowed items-center rounded-lg bg-slate-300 px-5 py-2.5 text-sm font-medium text-white"
            >
              Run review
            </button>
